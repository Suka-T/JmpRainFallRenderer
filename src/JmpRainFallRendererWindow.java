...
public class JmpRainFallRendererWindow extends JFrame implements MouseListener, MouseMotionListener, MouseWheelListener, Runnable {
     ...
     * Create the frame.
     */
    public JmpRainFallRendererWindow() {
        this.setTransferHandler(new DropFileHandler());
        this.setTitle("JMP Rain Fall Renderer");
        setLocation(10, 10);
    ...
    public void paintDisplay(Graphics g) {
        ...
        if (JMPCoreAccessor.getSystemManager().getStatus(ISystemManager.SYSTEM_STATUS_ID_FILE_LOADING) == true) {
            ...
            if (imageWorkerMgr.getNotesImage() == null) {
                ...
                Graphics2D lotG2d = (Graphics2D) g2d.create();
                ...
                lotG2d.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
        ...
        if (tickBarPosition > 0) {
            ...
            for (int i=0; i<aHakken.length; i++) {
                ...
                if (keyFocus != -1) {
                    ...
                    g.fill3DRect(aHakken[i].x, aHakken[i].y, aHakken[i].width, aHakken[i].height, true);
                    g.setColor(Color.LIGHT_GRAY);
                    g.fill3DRect(aHakken[i].x, aHakken[i].y, (int) (aHakken[i].width * 0.05), aHakken[i].height, true);
                    g.setColor(Color.GRAY);
                    g.drawRect(aHakken[i].x, aHakken[i].y, (int) (aHakken[i].width * 0.05), aHakken[i].height);
                    g.setColor(Color.LIGHT_GRAY);
                    g.drawRect(aHakken[i].x, aHakken[i].y, aHakken[i].width, aHakken[i].height);
                }
    ...
    public void catchLoadItem(Object item) {
        ...
        if ((files != null) && (files.size() > 0)) {
            if (files.size() >= 2) {
                ...
                if (Utility.checkExtensions(path, JmpRainFallRenderer.Extensions.split(",")) == true) {
